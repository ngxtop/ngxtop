...
def get_log_formats(config):
    ...
    log_format = (
        Literal("log_format") + parameter + Group(OneOrMore(parameter)) + semicolon
    )
...
def detect_log_config(arguments):
    ...
    log_path = choose_one(
        list(access_logs.keys()), "Select access log file to process: "
    )
    format_name = access_logs[log_path]
    if format_name not in log_formats:
        error_exit(
            'Incorrect format name set in config for access log file "%s"' % log_path
        )
    return log_path, log_formats[format_name]
